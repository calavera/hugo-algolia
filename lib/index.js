...
function HugoAlgolia(options) {
  ...
  if (process.argv.indexOf("--imap") > 0) {
    this.imageMapFile = process.argv[process.argv.indexOf("--imap") + 1];
    this.imageLabel = process.argv[process.argv.indexOf("--ilabel") + 1];

    if (this.imageMapFile !== undefined) {
      const contents = fs.readFileSync(this.imageMapFile);
      this.imageMapJSON = JSON.parse(contents);
    }
  }
  ...
  HugoAlgolia.prototype.sendIndex = function(indObj, name) {
    ...
    tmpIndex.addObjects(indexToSend, (err, content) => {
      ...
      tmpIndex.waitTask(content.taskID, err => {
        if (err) {
          console.log(err);
          return;
        }
  ...
  HugoAlgolia.prototype.formatDataInFile = function(meta, filePath) {
    ...
    if (meta.data) {
      for (prop in meta.data) {
        if (prop === self.imageLabel && self.imageMapJSON !== undefined) {
          var data = self.imageMapJSON[meta.data[prop]];
  ...
  HugoAlgolia.prototype.readFile = function(filePath) {
    ...
    // Try reindexing with toml delimiters if there
    // is no title.
    if (!meta.data.title) {
      meta = matter.read(filePath, {
        language: "toml",
        delims: "+++",
        engines: {
          toml: toml.parse.bind(toml)
    ...
    if (meta.data.index === false || meta.data.draft === true) {
      return
    }
